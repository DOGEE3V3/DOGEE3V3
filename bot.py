...
class DiscordTelegramSoundBot:
    ...
    def _tg_role_keyboard(self, guild: discord.Guild) -> ReplyKeyboardMarkup:
        ...
        buttons.append([KeyboardButton("Без роли")])
    ...
    async def tg_choose_role(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if text == "Без роли":
            context.user_data["selected_role_id"] = NO_ROLE_KEY
            context.user_data["selected_role_name"] = "Без роли"
            await update.message.reply_text(
                "Выбран сценарий «Без роли». Теперь отправьте аудиофайл или voice-сообщение, либо нажмите «Назад».",
        ...
        if not match:
            await update.message.reply_text(
                "Пожалуйста, нажмите кнопку роли из списка, «Без роли» или «Назад».",
            )
            return CHOOSE_ROLE
        ...
        if role is None:
            await update.message.reply_text(
                "Роль не найдена. Выберите роль кнопкой из списка, «Без роли» или нажмите «Назад».",
    ...
    def _find_sound_for_member(self, member: discord.Member) -> Optional[str]:
        ...
        if not has_custom_roles:
            return self.storage.get_role_sound(NO_ROLE_KEY)
        return None
