...
class DiscordTelegramSoundBot:
    ...
    def _register_telegram_handlers(self) -> None:
        ...
        conv = ConversationHandler(
            entry_points=[
                ...
                MessageHandler(filters.Regex(r"^Звук$"), self.tg_sound_entry),
            ],
            states={
                CHOOSE_ROLE: [
    ...
    async def tg_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            "Привет! Используйте кнопки: 1) Роль 2) Звук",
            reply_markup=self._tg_keyboard(),
        )

    async def tg_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        await update.message.reply_text(
            "1. Роль — посмотреть роли сервера.\n"
            "2. Звук — выбрать роль и загрузить аудио.",
            reply_markup=self._tg_keyboard(),
        )
    ...
    async def tg_role_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        role_lines = [f"{role.id} — {role.name}" for role in sorted(guild.roles, key=lambda r: r.position, reverse=True) if role.name != "@everyone"]
        await update.message.reply_text(
            "Роли сервера:\n" + "\n".join(role_lines),
            reply_markup=self._tg_keyboard(),
        )
    ...
    async def tg_sound_entry(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        if update.effective_user is None or not self._tg_is_admin(update.effective_user.id):
            await update.message.reply_text("Доступ запрещен.")
            return ConversationHandler.END
    ...
    async def tg_receive_audio(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if not role_id:
            await update.message.reply_text("Сначала выберите роль через кнопку «Звук».")
            return ConversationHandler.END

        audio = update.message.audio
        voice = update.message.voice
        if audio is None and voice is None:
            await update.message.reply_text("Отправьте аудиофайл или voice.")
            return WAIT_AUDIO
