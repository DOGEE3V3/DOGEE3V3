...
- Telegram-бот с основными кнопками:
  1. `Роль` — показать роли сервера.
  2. `Звук` — выбрать роль и загрузить аудио, которое назначается на эту роль.
...
1. Нажмите `Звук`.
2. Отправьте ID роли (или нажмите кнопку `Назад`, если передумали).
3. Отправьте аудиофайл или voice-сообщение (кнопка `Назад` также доступна).
4. Бот сохранит файл и привяжет его к роли.
